...
public class RecordTypeInfoWriter
{
    ...
    {
        var str = string.Join(string.Empty, variables.Where(x => x.ValueLength > 255).Select(x => $"{x.ShortName}={x.ValueLength}\0\t"));
    ...
    {
        foreach (var longStringValueLabels in records.Where(x => x.FormatType == FormatType.A && x.ValueLabels != null && x.ValueLabels.Any() && x.ValueLength > 8)) WriteValueLabelStringRecord(longStringValueLabels);
    }
    ...
    {
        foreach (var nonStringValueLabels in records.Where(x => x.FormatType == FormatType.A && x.MissingValueType != MissingValueType.NoMissingValues && x.ValueLength > 8)) WriteMissingStringRecord(nonStringValueLabels);
    }
    ...
    private void WriteMissingStringRecord(VariableWrapper variable)
    {
        ...
        _writer.Write(8);
        foreach (var missing in variable.MissingValuesObject) _writer.Write(_encoding.GetPaddedValueAsByteArray((string)missing, 8));
    }
}